				...
				<c:choose>
					...
					<c:otherwise>
						<div id="coin">
							...
							<div class="coin-1">구매개수</div>
							<div class="coin-1">구매금액</div>
							<div class="coin-1">코인시세</div>
							<div class="coin-1">구매날짜</div>
						</div>
						<div id="coin">
							<c:set var="coin_count" value="0" />
							<c:forEach var="item" items="${buyMap.buylist}">
								<c:set var="coin_count" value="${coin_count+1 }" />
								<div class="coin-2">${item.coinTitle}</div>
								<div class="coin-2">${item.buyAmount}</div>
								<div class="coin-2">${item.needMoney}</div>
								<div class="coin-2">${item.buyPrice}</div>
								<div class="coin-2">${item.buyDate}</div>
							</c:forEach>
						</div>
					</c:otherwise>
				</c:choose>
