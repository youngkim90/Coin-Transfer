...
<script type="text/javascript">
	function fn_buycoin(coinNum, coinName, coinPrice, coinTitle) {
		var formObj = document.createElement("form");
		var coin_num = document.createElement("input");
		var coin_name = document.createElement("input");
		var coin_price = document.createElement("input");
		var coin_title = document.createElement("input");

		coin_num.name = "coinNum";
		coin_name.name = "coinName";
		coin_price.name = "coinPrice";
		coin_title.name = "coinTitle";

		coin_num.value = coinNum;
		coin_name.value = coinName;
		coin_price.value = coinPrice;
		coin_title.value = coinTitle;

		formObj.appendChild(coin_num);
		formObj.appendChild(coin_name);
		formObj.appendChild(coin_price);
		formObj.appendChild(coin_title);

		document.body.appendChild(formObj);
		formObj.method = "post";
		formObj.action = "${contextPath}/account/buyCoin2.do";
		formObj.submit();
					...
					<c:otherwise>
						...
						<c:forEach var="item" items="${coinMap.coininfo}">
							...
							<div class="coin">
								...
								<div>
									<a href="javascript:fn_buycoin('${item.coinNum}','${item.coinName}','${item.coinPrice}','${item.coinTitle}');" class="btn-gradient cyan">코인구매</a>
								</div>
							</div>
